...
import { remainingMacros } from "@/lib/macros";
...
import type { MealSuggestion, MacroTargets, MacroTotals } from "@/types/domain";
...
const clampPrepTime = (value: number): number =>
  Math.min(Math.max(value, 1), 240);
...
const loadContext = async (): Promise<SuggestionsContext> => {
  ...
  return {
    ...
    totals: data.totals,
    remaining: remainingMacros(data.targets, data.totals),
  };
};

const contextQuery = useQuery({
  queryKey: computed(() => [
    ...queryKeys.suggestionsContext,
    activeDayStore.activeDate,
  ]),
  queryFn: async () => {
    const result = await loadContext();
    activeDayStore.setActiveDate(result.activeDate);
...
const contextError = computed(
  () => (contextQuery.error.value as Error | null) ?? null,
);
...
const exceedWarning = (
  ...
  if (suggestion.calories > remaining.calories) {
    parts.push(
      `calories by ${Math.round(suggestion.calories - remaining.calories)} kcal`,
    );
  }
  if (suggestion.protein > remaining.protein) {
    parts.push(
      `protein by ${Math.round(suggestion.protein - remaining.protein)} g`,
    );
  }
...
const regenerateIngredients = async (): Promise<void> => {
  await requestSuggestions(
    `Adjust ingredients: ${ingredientNotes.value.trim()}`,
  );
};
...
const regenerateTime = async (): Promise<void> => {
  ...
  await requestSuggestions(
    `Adjust prep time to ${maxPrepTimeMinutes.value} minutes`,
  );
};
...
const refreshContext = async (): Promise<void> => {
  await contextQuery.refetch();
  contextRefreshedAt.value = new Date();
  await queryClient.invalidateQueries({ queryKey: queryKeys.daySummary });
};
</script>
  ...
  <section class="app-page feature feature-suggestions">
    ...
    <Card class="glass space-y-4 p-3 sm:p-5">
      <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div class="stat-chip">
          <p
            class="text-[11px] uppercase tracking-[0.04em] text-muted-foreground"
          >
            Calories
          </p>
          <p class="mt-1 text-base font-semibold sm:text-lg">
            {{ contextData?.remaining.calories ?? 0 }} kcal
          </p>
        </div>
        <div class="stat-chip">
          <p
          ...
          <p class="mt-1 text-base font-semibold sm:text-lg">
            {{ contextData?.remaining.protein ?? 0 }} g
          </p>
        </div>
        <div class="stat-chip">
          <p
          ...
          <p class="mt-1 text-base font-semibold sm:text-lg">
            {{ contextData?.remaining.carbs ?? 0 }} g
          </p>
        </div>
        <div class="stat-chip">
          <p
      ...
      <p
        v-if="
          contextData &&
          contextData.targets.calories === 0 &&
          contextData.targets.protein === 0 &&
      ...
      <div class="flex items-center gap-3">
        <Button variant="ghost" size="sm" @click="refreshContext"
          >Refresh remaining macros</Button
        >
        <span v-if="contextAgeLabel" class="text-xs text-muted-foreground"
          >as of {{ contextAgeLabel }}</span
        >
      </div>
    </Card>
    ...
    >
      ...
      <ul class="space-y-1 text-xs text-muted-foreground">
        <li>
          🍽 <strong>Meal type</strong> — Choose a full meal or a lighter snack
        </li>
        <li>
          ⏱ <strong>Max prep time</strong> — How many minutes you can spare to
          cook
        </li>
        <li>
          🥗 <strong>Ingredient notes</strong> — Any restrictions or preferences
          (optional)
        </li>
      </ul>
      <p class="text-xs text-muted-foreground">
        Tap <strong>Get suggestions</strong> and the AI will propose 3 options
        that fit your remaining macros.
      </p>
    </div>
    ...
    <Card class="space-y-4 p-3 sm:p-5">
      <div class="grid gap-3 sm:grid-cols-2">
        ...
        <div class="space-y-1">
          <label
            class="text-xs font-medium uppercase tracking-[0.03em] text-muted-foreground"
            >Max prep time (minutes)</label
          >
          <div class="relative flex items-center gap-2">
            <Input
              v-model="maxPrepTimeMinutes"
              type="number"
              min="1"
              max="240"
              class="flex-1"
      ...
      <div class="space-y-1">
        <label
          class="text-xs font-medium uppercase tracking-[0.03em] text-muted-foreground"
          >Ingredient notes</label
        >
        <Textarea
          v-model="ingredientNotes"
          :rows="3"
          placeholder="Restrictions or preferences, e.g. no peanuts, dairy-free"
        />
        <p class="text-xs text-muted-foreground">
          List any dietary restrictions or preferred ingredients. Leave blank
          for no preference.
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <Button
          :loading="fetchSuggestionsMutation.isPending.value"
    ...
    <div class="space-y-3">
      <Card
        ...
        class="rounded-2xl border-dashed p-3 text-sm text-muted-foreground sm:p-4"
      >
      ...
      >
        ...
        <p v-if="suggestion.warning" class="text-xs text-muted-foreground">
          Consider halving the portion or choosing a lighter option.
        </p>
        <p v-if="suggestion.notes" class="text-xs text-muted-foreground">
          {{ suggestion.notes }}
        </p>
      </Card>
    </div>
